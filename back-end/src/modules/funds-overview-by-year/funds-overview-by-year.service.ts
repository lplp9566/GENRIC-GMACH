...
export class FundsOverviewByYearService {
  constructor(
    ...
    @InjectRepository(FundsOverviewByYearViewEntity)
    private readonly fundsOverviewByYearViewRepo: Repository<FundsOverviewByYearViewEntity>,
  ) {}

  async getOrCreateOverview(year: number){
    let record = await this.fundsOverviewByYearViewRepo.findOne({
      where: { year },
    });
    // if (!record) {
    //   record = this.fundsOverviewByYearViewRepo.create({
    //     year,
    ...
    //     total_expenses: 0,
    //   });
    //   await this.fundsOverviewByYearViewRepo.save(record);
    // }
    return record;
  }
  async getAllFundsOverview(): Promise<FundsOverviewByYearViewEntity[]> {
    return this.fundsOverviewByYearViewRepo.find({ order: { year: 'ASC' } });
  }
